...
def visualize_label(img, boxes, path, color=(0, 255, 0)):
    ...
    #xmin, ymin = boxes.min(1)[0]
    #xmax, ymax = boxes.max(1)[0]
    #x,y,w,h = boxes[0][1][0],boxes[0][1][1],xmax-xmin,ymax-ymin
...
if __name__ == '__main__':
    ...
    parser.add_argument('--result-path', type=str, default='./VGG_result', help='Input image path')
    ...
    for num, image_path in enumerate(sorted(image_list)):
        ...
        try:
            ...
            grad_cam = GradCam(model=models.vgg19(pretrained=True), target_layer_names=args.target_layer, device=device)
            ...
            for i, bbox in enumerate(bboxs):
                if i == 0:
                    ...
                    utils.save_image(torch.from_numpy(gb), os.path.join(result_img_dir, "top_" + str(i) + "_" + image_name + "_gb.jpg"))
                    cam_mask = np.zeros(gb.shape)
                    for i in range(0, gb.shape[0]):
                        cam_mask[i, :, :] = bbox["mask"]
                    cam_gb = np.multiply(cam_mask, gb)
                    utils.save_image(torch.from_numpy(cam_gb), os.path.join(result_img_dir, "top_" + str(i) + "_" + image_name + "_cam_gb.jpg"))
